...
![Version: 3.5.0](https://img.shields.io/badge/Version-3.5.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 2.6.3](https://img.shields.io/badge/AppVersion-2.6.3-informational?style=flat-square)

Innago Helm chart for deploying web applications to Kubernetes with production-ready patterns

**Homepage:** <https://innago-property-management.github.io/helm-charts/webapp>
...
| autoscaling.targetCPUUtilizationPercentage | int | `80` | Target CPU utilization percentage (scale up when exceeded). Example: 80 means scale up when average CPU > 80%. To scale on memory only, set this to null, 0 or false. At least one of the CPU and memory targets must remain set; disabling both fails rendering |
...
| vpa | object | `{"annotations":{},"controlledResources":null,"controlledValues":"RequestsAndLimits","enabled":false,"maxAllowed":{"cpu":4,"memory":"8Gi"},"minAllowed":{"cpu":"10m","memory":"32Mi"},"updateMode":"Off"}` | Requires VPA to be installed in the cluster |
| vpa.annotations | object | `{}` | Example: description annotations, external tool integrations |
| vpa.controlledResources | string | `nil` | Options: ["cpu", "memory"] or ["memory"] |
| vpa.controlledValues | string | `"RequestsAndLimits"` | RequestsOnly: VPA only manages requests |
| vpa.enabled | bool | `false` | Enable VPA (creates VerticalPodAutoscaler resource) |
| vpa.maxAllowed | object | `{"cpu":4,"memory":"8Gi"}` | Maximum resource values VPA can recommend |
| vpa.minAllowed | object | `{"cpu":"10m","memory":"32Mi"}` | Minimum resource values VPA can recommend |
| vpa.updateMode | string | `"Off"` | Note: "Auto" is deprecated, use "InPlaceOrRecreate" instead |
